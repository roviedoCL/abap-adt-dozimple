...
| Trazabilidad | Una escritura sin rastro, o un rastro retocado | **Registro de auditoría** local encadenado por hash (`audit.jsonl`): intención, resultado y denegaciones de toda escritura y ejecución, con usuario SAP, sistema, forma de confirmación y huella sha256 del contenido (nunca el contenido). Sin registro no se escribe; escrituras de varios servidores a la vez, serializadas con bloqueo. `npm run audit:verify` detecta ediciones y borrados puntuales. **Límite:** la cadena no lleva secreto, así que quien tenga la cuenta del usuario podría reescribirla entera; para detectarlo, anota fuera del equipo el «último hash» que imprime `audit:verify` |
...
| Configuración | Alguien redirige el servidor, habilita la escritura o cambia el comando de un componente | `systems.json` se rechaza si lo pueden modificar grupo u otros usuarios, o si es de otro usuario (**en Windows esta comprobación no se aplica y el servidor lo avisa en cada arranque**); el sistema por defecto por variable de entorno se identifica como tal en cada respuesta, y si no existe es error |
