...
| **T**ampering | Retocar el registro de auditoría | Cadena de hash verificable; bloqueo entre procesos para que la concurrencia no rompa la cadena; `audit:verify` imprime el último hash para anclarlo fuera del equipo | La cadena no lleva secreto: con la cuenta del usuario se puede reescribir entera y seguir «íntegra». Una clave HMAC local no lo resolvería (el mismo usuario la leería); lo que lo resuelve es anclar el último hash fuera (roadmap: envío a un almacén de solo añadir) |
