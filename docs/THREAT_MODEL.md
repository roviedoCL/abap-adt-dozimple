...
| **T**ampering | Cambiar `systems.json` para habilitar escritura, redirigir a otro host o cambiar el comando de un componente (ejecución de código) | Rechazo si es modificable por otros o de otro usuario: permisos en macOS/Linux, ACL por SID en Windows; si no se puede comprobar, no se arranca | Un proceso del propio usuario puede modificarlo |
| **T**ampering | Retocar el registro de auditoría | Cadena de hash + firma HMAC con clave en el llavero del SO (fecha de creación incluida: quitar firmas se detecta); bloqueo entre procesos; `audit:verify` imprime el último hash para anclarlo fuera | Quien pueda leer el llavero del usuario (su propia sesión, sin `--strict`) podría recalcularlo; el anclaje externo del último hash cubre ese caso. En Windows, sin almacén, solo cadena |
| **T**ampering | Una escritura aprobada se aplica sobre una versión distinta de la mostrada | La huella de lo mostrado viaja con la confirmación y se compara bajo el bloqueo del objeto; si cambió, no se escribe | — |
...
|---|---|---|
| LLM01 Prompt injection | Comentarios en fuente ABAP, textos, datos de tablas, documentación o posts de comunidad con instrucciones | Marca de «dato, nunca instrucciones» en toda respuesta (probabilística: viaja en el mismo canal que los datos y el contenido podría imitarla; el protocolo MCP aún no separa instrucciones de datos); marcado neutralizado tras decodificar HTML; **la barrera efectiva**: ninguna escritura sin confirmación humana, tools de escritura destructivas para el cliente, y no existen tools para liberar, importar ni borrar |
